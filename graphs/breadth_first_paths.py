...
from fundamentals import Queue
from fundamentals import Stack
from stdlib import writef, write, writeln
...
class BreadthFirstPaths:
    ...
    def _check(self, G, s):
        ...
        if self._dist_to[s] != 0:
            writef("distance of source %i to itself = %i\n", s, self._dist_to[s])
            return False
...
if __name__ == "__main__":
    ...
    for v in range(G.V()):
        if bfs.has_path_to(v):
            writef("%d to %d (%d):  ", s, v, bfs.dist_to(v))
